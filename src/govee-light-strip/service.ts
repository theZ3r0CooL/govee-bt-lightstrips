...
import { Color, colorToHex, xorColor, hexify } from "@/util/color-util";
...
const sendHexMessage = async (lightStrip: GoveeLightStrip, message: string) => {
  await lightStrip.writeCharacteristic.writeAsync(
    Buffer.from(message, "hex"),
    false,
  );
};

function assembleMessageWithChecksum(
  ...
  let checksum =
    ...
    xorColor(whiteColor);
  return (
    hexify(constants.CONTROL_PACKET_ID) +
    hexify(messageType) +
    hexify(specialByte) +
    colorToHex(rgbColor) +
    hexify(flag) +
...
export const setLightStripBrightness = (
  lightStrip: GoveeLightStrip,
  newVal: number,
): GoveeLightStrip => {
  sendHexMessage(
    lightStrip,
    assembleMessageWithChecksum(
      MessageType.Brightness,
      newVal,
      constants.EMPTY_COLOR,
      0x0,
      constants.EMPTY_COLOR,
    ),
  );
...
export const setLightStripPower = (
  lightStrip: GoveeLightStrip,
  newVal: boolean,
): GoveeLightStrip => {
  let flag = newVal ? 1 : 0;
...
export const setLightStripColor = (
  lightStrip: GoveeLightStrip,
  newColor: Color,
  isWhite: boolean,
): GoveeLightStrip => {
